...
- `backend/server.js` (Express, port 4000) — tournament indexing, IPFS pinning (mock), oracle verification (HMAC), JWT auth, notifications, balances.
...
- After changing the contract: `npm run compile` → re-copy ABI → `npm test` (root) → `npm run build` + `npm test` (frontend).
